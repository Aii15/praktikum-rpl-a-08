...
const previewGalleryContainer = document.getElementById(
    "previewGalleryContainer",
);
const liveLayoutGallery = document.getElementById("liveLayoutGallery");
const hiddenPositionsContainer = document.getElementById(
...
function handleFiles(files) {
    ...
    for (let file of newFiles) {
        ...
        if (file.size > maxLimit) {
            ...
            oversizedFileNames.push(
                `${file.name} (> batas server PHP ${phpLimitMB}MB)`,
            );
            continue;
        }
    ...
    if (hasOversizedFile) {
        ...
        if (typeof window.showProfileToast === "function") {
            window.showProfileToast(
                `Ukuran file foto terlalu besar. File ditolak: ${namesText}. Silakan upload gambar dengan file size yang lebih kecil (MAX 10 MB)`,
            );
        } else {
            alert(`Ukuran file foto terlalu besar. File ditolak: ${namesText}`);
        }
    }
}
...
function updateFormInputsAndPreviews() {
    ...
    if (window.selectedFiles.length > 0) {
        ...
        countLabel.textContent =
            "Belum ada foto terpilih (Minimal 2, Maksimal 5)";
    }
        ...
        window.selectedFiles.forEach((item, index) => {
            ...
            function updateImagePositions() {
                ...
                if (
                    ...
                    hiddenPositionsContainer.children &&
                    hiddenPositionsContainer.children[index]
                ) {
...
if (propertyForm) {
    propertyForm.addEventListener("submit", function (e) {
        if (window.selectedFiles.length < 2) {
            e.preventDefault();
            showProfileToast(
                "Minimal 2 foto wajib diunggah untuk melanjutkan.",
            );
            return;
        ...
        if (totalSize > phpPostLimit) {
            ...
            showProfileToast(
                `Total ukuran file (${(totalSize / (1024 * 1024)).toFixed(1)}MB) melebihi batas server PHP (${limitMB}MB). Silakan kurangi jumlah foto atau perkecil resolusi foto.`,
            );
            return;
        }
    });
